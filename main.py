...
CHANNEL_ID = os.environ.get("CHANNEL_ID", "UCm6dEXyAMIy0njEOW-suLww")
...
def main():
    ...
    if not os.path.exists(filename):
        print(f"جارٍ تحميل الفيديو {video_url}")
        download_video(video_url, filename)
        output_pattern = f"{video_id}_part%03d.mp4"
        print("جارٍ تقطيع الفيديو وعمل العلامات ...")
        cut_and_mark_video(filename, output_pattern)

        parts = sorted([f for f in os.listdir('.') if f.startswith(f"{video_id}_part")])
        for part in parts:
            print(f"إرسال الجزء {part} إلى التليجرام...")
            send_to_telegram(part)
    else:
        print(f"الفيديو {filename} موجود مسبقاً")
